import { User, ChevronDown, LogIn, Shield, LogOut, Search, Menu, X } from "lucide-react"
                ...
                {link.submenu?.items ? (
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="ghost"
                        className="text-white/70 hover:text-white bg-transparent px-4 py-2 font-medium text-sm transition-colors duration-200 h-auto hover:bg-white/5 rounded-lg"
                      >
                        {link.label}
                        <ChevronDown className="ml-1.5 h-3.5 w-3.5 text-white/50" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent 
                      align="start" 
                      className="z-dropdown w-72 p-3 bg-black/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-xl"
                    >
                      <div className="space-y-1">
                        {link.submenu.items.map((item, itemIndex) => (
                          ...
                          >
                            <div className="space-y-1">
                              <div className="font-medium text-white transition-colors group-hover:text-emerald-300">
                                {item.label}
                              </div>
                              {item.description && (
                  ...
                  <a
                    ...
                    className="text-white/70 hover:text-white px-4 py-2 font-medium text-sm transition-colors duration-200 hover:bg-white/5 rounded-lg"
                  >
                    {link.label}
                  </a>
                )}
              ...
              <Popover>
                ...
                <PopoverContent align="end" className="z-dropdown w-64 p-3 bg-black/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-xl">
                  <div className="space-y-2">
                    <div className="px-3 py-2 text-sm font-semibold text-white border-b border-white/10">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 border-t border-white/10 space-y-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    ...
                    onClick={toggleLanguage}
                  >
                    <span className="text-sm">{t('header.buttons.language')}</span>
                  </Button>
                </div>
              </div>
              ...
              {!user && (
                <div className="pt-4 border-t border-white/10 space-y-2">
                  ...
                  >
                    <User className="h-4 w-4 mr-2" />
                    <span>{t('header.buttons.createAccount')}</span>
                  </Button>
                  <Button 
                    variant="outline" 
                    size="sm"
                    className="w-full bg-white text-black hover:bg-white/90 border-white"
                    onClick={handleOpenLogin}
                  >
                    <LogIn className="h-4 w-4 mr-2" />
