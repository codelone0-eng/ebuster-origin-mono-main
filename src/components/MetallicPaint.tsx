...
export default function MetallicPaint({
  ...
  function renderFallback() {
    ...
    ctx.fillText('EBUSTER', canvas.width / 2, canvas.height / 2);
  }
  ...
  useEffect(() => {
    function initShader() {
      ...
      // Устанавливаем прозрачный фон canvas
      ...
      if (!program || !vertexShader || !fragmentShader) {
        console.error('Failed to create shaders or program');
        return;
      }
      ...
      // Устанавливаем базовые uniforms
      gl.uniform1f(uniforms.u_time, 0);
      gl.uniform1f(uniforms.u_patternScale, params.patternScale);
      gl.uniform1f(uniforms.u_refraction, params.refraction);
      gl.uniform1f(uniforms.u_edge, params.edge);
      ...
      console.log('MetallicPaint initialized successfully');
    }
  ...
  useEffect(() => {
    if (!gl || !uniforms || !isInitialized) return;
    updateUniforms();
  }, [gl, params, uniforms, isInitialized]);

  useEffect(() => {
    if (!gl || !uniforms || !isInitialized || !hasTexture) return;
    ...
    function render(currentTime: number) {
      ...
      // Очищаем canvas перед рендером
      gl!.clear(gl!.COLOR_BUFFER_BIT);
      gl!.drawArrays(gl!.TRIANGLE_STRIP, 0, 4);
      
      // Проверяем на ошибки WebGL
  ...
  }, [gl, params.speed, isInitialized, hasTexture]);

  useEffect(() => {
    const canvasEl = canvasRef.current;
    if (!canvasEl || !gl || !uniforms || !isInitialized) return;
    ...
    function resizeCanvas() {
      ...
      // Принудительный рендер после изменения размера
      gl.clear(gl.COLOR_BUFFER_BIT);
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
      
      console.log('Canvas resized:', canvasEl.width, 'x', canvasEl.height, 'ratio:', imgRatio);
    }
  ...
  }, [gl, uniforms, imageData, isInitialized]);

  useEffect(() => {
    if (!gl || !uniforms || !isInitialized || !imageData) return;

    console.log('Loading texture:', imageData.width, 'x', imageData.height);
    ...
    try {
      ...
      // Принудительный рендер после загрузки текстуры
      gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    } catch (e) {
      console.error('Error uploading texture:', e);
      // Fallback рендер при ошибке
      renderFallback();
    }
  ...
  useEffect(() => {
    ...
    // Fallback рендер если WebGL не работает
    if (!isInitialized) {
  ...
  return (
    <canvas 
      ...
      style={{ 
        backgroundColor: hasTexture ? 'transparent' : '#666666',
        display: 'block',
        position: 'relative',
        zIndex: 1,
        minWidth: '100%',
