/**
 * Централизованные переводы для уведомлений
 * Все уведомления в приложении должны использовать эти переводы
 */

export const notificationTranslations = {
  // === АВТОРИЗАЦИЯ ===
  auth: {
    // Успешные операции
    loginSuccess: {
      title: "Вход выполнен",
      description: "Добро пожаловать!"
    },
    registerSuccess: {
      title: "Регистрация успешна",
      description: "Проверьте email для подтверждения аккаунта"
    },
    logoutSuccess: {
      title: "Выход выполнен",
      description: "До свидания!"
    },
    passwordResetSent: {
      title: "Ссылка отправлена",
      description: "Проверьте email для сброса пароля"
    },
    passwordResetSuccess: {
      title: "Пароль изменен",
      description: "Теперь вы можете войти с новым паролем"
    },
    emailConfirmed: {
      title: "Email подтвержден",
      description: "Аккаунт успешно активирован"
    },
    emailSendError: {
      title: "Ошибка отправки email",
      description: "Не удалось отправить письмо подтверждения"
    },

    // Ошибки
    invalidCredentials: {
      title: "Ошибка входа",
      description: "Неверный email или пароль"
    },
    emailAlreadyExists: {
      title: "Ошибка регистрации",
      description: "Пользователь с таким email уже существует"
    },
    weakPassword: {
      title: "Слабый пароль",
      description: "Пароль должен содержать минимум 6 символов"
    },
    invalidEmail: {
      title: "Неверный email",
      description: "Введите корректный email адрес"
    },
    userNotFound: {
      title: "Пользователь не найден",
      description: "Аккаунт с таким email не существует"
    },
    tooManyRequests: {
      title: "Слишком много попыток",
      description: "Попробуйте позже"
    },
    networkError: {
      title: "Ошибка сети",
      description: "Проверьте подключение к интернету"
    },
    sessionExpired: {
      title: "Сессия истекла",
      description: "Войдите в аккаунт заново"
    }
  },

  // === АДМИН ПАНЕЛЬ ===
  admin: {
    // Успешные операции
    userBanned: {
      title: "Пользователь забанен",
      description: "Пользователь успешно заблокирован"
    },
    userUnbanned: {
      title: "Пользователь разбанен",
      description: "Блокировка снята"
    },
    userDeleted: {
      title: "Пользователь удален",
      description: "Аккаунт пользователя удален"
    },
    userRoleUpdated: {
      title: "Роль изменена",
      description: "Роль пользователя обновлена"
    },
    subscriptionUpdated: {
      title: "Подписка обновлена",
      description: "Тип подписки изменен"
    },
    passwordGenerated: {
      title: "Пароль сгенерирован",
      description: "Новый пароль создан"
    },
    passwordCopied: {
      title: "Пароль скопирован",
      description: "Пароль скопирован в буфер обмена"
    },
    dataExported: {
      title: "Данные экспортированы",
      description: "Файл успешно сохранен"
    },
    settingsSaved: {
      title: "Настройки сохранены",
      description: "Изменения применены"
    },

    // Ошибки
    banUserError: {
      title: "Ошибка блокировки",
      description: "Не удалось заблокировать пользователя"
    },
    unbanUserError: {
      title: "Ошибка разблокировки",
      description: "Не удалось снять блокировку"
    },
    deleteUserError: {
      title: "Ошибка удаления",
      description: "Не удалось удалить пользователя"
    },
    updateRoleError: {
      title: "Ошибка обновления роли",
      description: "Не удалось изменить роль"
    },
    generatePasswordError: {
      title: "Ошибка генерации",
      description: "Не удалось сгенерировать пароль"
    },
    copyPasswordError: {
      title: "Ошибка копирования",
      description: "Не удалось скопировать пароль"
    },
    exportDataError: {
      title: "Ошибка экспорта",
      description: "Не удалось экспортировать данные"
    },
    insufficientPermissions: {
      title: "Недостаточно прав",
      description: "У вас нет прав для выполнения этого действия"
    }
  },

  // === ОБЩИЕ УВЕДОМЛЕНИЯ ===
  common: {
    // Успешные операции
    saved: {
      title: "Сохранено",
      description: "Изменения успешно сохранены"
    },
    updated: {
      title: "Обновлено",
      description: "Данные обновлены"
    },
    deleted: {
      title: "Удалено",
      description: "Элемент удален"
    },
    copied: {
      title: "Скопировано",
      description: "Скопировано в буфер обмена"
    },
    uploaded: {
      title: "Загружено",
      description: "Файл успешно загружен"
    },
    sent: {
      title: "Отправлено",
      description: "Сообщение отправлено"
    },

    // Ошибки
    error: {
      title: "Ошибка",
      description: "Произошла ошибка"
    },
    networkError: {
      title: "Ошибка сети",
      description: "Проверьте подключение к интернету"
    },
    serverError: {
      title: "Ошибка сервера",
      description: "Попробуйте позже"
    },
    validationError: {
      title: "Ошибка валидации",
      description: "Проверьте введенные данные"
    },
    permissionDenied: {
      title: "Доступ запрещен",
      description: "У вас нет прав для этого действия"
    },
    notFound: {
      title: "Не найдено",
      description: "Запрашиваемый элемент не найден"
    },
    timeout: {
      title: "Превышено время ожидания",
      description: "Операция заняла слишком много времени"
    }
  },

  // === ФОРМЫ ===
  forms: {
    requiredField: {
      title: "Обязательное поле",
      description: "Заполните все обязательные поля"
    },
    invalidFormat: {
      title: "Неверный формат",
      description: "Проверьте формат введенных данных"
    },
    passwordMismatch: {
      title: "Пароли не совпадают",
      description: "Пароли должны быть одинаковыми"
    },
    fileTooLarge: {
      title: "Файл слишком большой",
      description: "Размер файла превышает допустимый"
    },
    invalidFileType: {
      title: "Неверный тип файла",
      description: "Выберите файл правильного типа"
    }
  }
} as const;

/**
 * Типы для уведомлений
 */
export type NotificationKey = 
  | keyof typeof notificationTranslations.auth
  | keyof typeof notificationTranslations.admin
  | keyof typeof notificationTranslations.common
  | keyof typeof notificationTranslations.forms;

/**
 * Функция для получения перевода уведомления
 */
export function getNotificationTranslation(
  category: 'auth' | 'admin' | 'common' | 'forms',
  key: string
): { title: string; description: string } {
  const translation = notificationTranslations[category][key as keyof typeof notificationTranslations[typeof category]];
  
  if (!translation) {
    console.warn(`Translation not found for ${category}.${key}`);
    return {
      title: "Уведомление",
      description: "Сообщение не найдено"
    };
  }
  
  return translation;
}

/**
 * Функция для получения перевода ошибки Supabase
 */
export function getSupabaseErrorTranslation(error: any): { title: string; description: string } {
  const errorMessage = error?.message?.toLowerCase() || '';
  
  // Анализируем сообщение об ошибке и возвращаем соответствующий перевод
  if (errorMessage.includes('invalid login credentials') || errorMessage.includes('invalid credentials')) {
    return notificationTranslations.auth.invalidCredentials;
  }
  
  if (errorMessage.includes('user not found')) {
    return notificationTranslations.auth.userNotFound;
  }
  
  if (errorMessage.includes('email already registered') || errorMessage.includes('user already registered')) {
    return notificationTranslations.auth.emailAlreadyExists;
  }
  
  if (errorMessage.includes('password should be at least') || errorMessage.includes('weak password')) {
    return notificationTranslations.auth.weakPassword;
  }
  
  if (errorMessage.includes('invalid email') || errorMessage.includes('email format')) {
    return notificationTranslations.auth.invalidEmail;
  }
  
  if (errorMessage.includes('too many requests') || errorMessage.includes('rate limit')) {
    return notificationTranslations.auth.tooManyRequests;
  }
  
  if (errorMessage.includes('network') || errorMessage.includes('connection')) {
    return notificationTranslations.auth.networkError;
  }
  
  if (errorMessage.includes('session') || errorMessage.includes('token')) {
    return notificationTranslations.auth.sessionExpired;
  }
  
  // Если не удалось определить тип ошибки, возвращаем общее сообщение
  return {
    title: "Ошибка",
    description: error?.message || "Произошла неизвестная ошибка"
  };
}
