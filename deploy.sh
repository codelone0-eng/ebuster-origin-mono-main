...
# ПРИНУДИТЕЛЬНАЯ МИГРАЦИЯ: Добавляем referer, если его нет
log "🔄 Миграция: добавление колонки referer..."
docker compose -f clickhouse/docker-compose.yml exec -T ebuster-clickhouse clickhouse-client --query "ALTER TABLE ebuster.access_logs ADD COLUMN IF NOT EXISTS referer String AFTER user_agent" 2>/dev/null || true
